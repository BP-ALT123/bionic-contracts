...
contract BionicPodManagerMock is IBionicPodManager, Test {
    ...
    function ownerToPod(address /*podOwner*/) external pure returns(IBionicPod) {
        return IBionicPod(address(0));
    }

    function getPod(address podOwner) external pure returns(IBionicPod) {
        return IBionicPod(podOwner);
    }
