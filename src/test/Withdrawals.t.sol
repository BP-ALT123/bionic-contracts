...
contract WithdrawalTests is DelegationTests {
    ...
    function testWithdrawalWrapper(
            ...
            fuzzedAddress(withdrawer) 
        {
            ...
            if(RANDAO) {
                _testWithdrawalAndDeregistration(operator, depositor, withdrawer, ethAmount, bionicAmount, withdrawAsTokens);
            }
            else{
                _testWithdrawalWithStakeUpdate(operator, depositor, withdrawer, ethAmount, bionicAmount, withdrawAsTokens);
            }
    ...
    function _testWithdrawalAndDeregistration(
        ...
        testDelegation(operator, depositor, ethAmount, bionicAmount);
    ...
    function testRedelegateAfterWithdrawal(
        ...
        testWithdrawalWrapper(operator, depositor, withdrawer, ethAmount, bionicAmount, withdrawAsShares, true);
    ...
    /// @param operator is the operator being delegated to.
    /// @param staker is the staker delegating stake to the operator.
    function testSlashedOperatorWithdrawal(address operator, address staker, uint96 ethAmount, uint96 bionicAmount)
        public
        fuzzedAddress(operator)
        fuzzedAddress(staker)
    {
        cheats.assume(staker != operator);
        testDelegation(operator, staker, ethAmount, bionicAmount);
