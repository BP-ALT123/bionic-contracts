...
contract DelegationUnitTests is BionicTestHelper {
    ...
    // @notice Emitted when a new operator registers in Bionic and provides their OperatorDetails.
    ...
    function setUp() override virtual public{
        ...
        delegationManager = DelegationManager(address(new TransparentUpgradeableProxy(address(emptyContract), address(BionicProxyAdmin), "")));
        ...
        BionicProxyAdmin.upgrade(TransparentUpgradeableProxy(payable(address(delegationManager))), address(delegationManagerImplementation));
        cheats.stopPrank();
        
        address initalOwner = address(this);
        uint256 initialPausedStatus = 0;
        delegationManager.initialize(initalOwner, BionicPauserReg, initialPausedStatus);
        ...
        addressIsExcludedFromFuzzedInputs[address(BionicProxyAdmin)] = true;
        ...
        require(delegationManager.pauserRegistry() == BionicPauserReg,
    ...
    function testCannotReinitializeDelegationManager() public {
        ...
        delegationManager.initialize(address(this), BionicPauserReg, 0);
    }
    ...
    // @notice Verifies that it is not possible for a staker to delegate to an operator when the operator is frozen in Bionic
