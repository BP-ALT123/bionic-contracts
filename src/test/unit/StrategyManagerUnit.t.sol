...
import "../mocks/BionicPodManagerMock.sol";
...
contract StrategyManagerUnitTests is Test, Utils {
    ...
    BionicPodManagerMock public bionicPodManagerMock;
    ...
    function setUp() virtual public {
        ...
        bionicPodManagerMock = new BionicPodManagerMock();
        strategyManagerImplementation = new StrategyManager(delegationMock, bionicPodManagerMock, slasherMock);
        ...
        // excude the zero address, the proxyAdmin and the bionicPodManagerMock from fuzzed inputs
        addressIsExcludedFromFuzzedInputs[address(0)] = true;
        addressIsExcludedFromFuzzedInputs[address(proxyAdmin)] = true;
        addressIsExcludedFromFuzzedInputs[address(bionicPodManagerMock)] = true;
    }
    ...
    function testDepositBeaconChainETHSuccessfully(address staker, uint256 amount) public filterFuzzedAddressInputs(staker) {
        ...
        cheats.startPrank(address(strategyManager.bionicPodManager()));
    ...
    function testDepositBeaconChainETHFailsWhenNotCalledByBionicPodManager(address improperCaller) public filterFuzzedAddressInputs(improperCaller) {
        uint256 amount = 1e18;
        address staker = address(this);

        cheats.expectRevert(bytes("StrategyManager.onlyBionicPodManager: not the bionicPodManager"));
    ...
    function testDepositBeaconChainETHFailsWhenDepositsPaused() public {
        ...
        cheats.startPrank(address(bionicPodManagerMock));
        strategyManager.depositBeaconChainETH(staker, amount);
        cheats.stopPrank();
    }
    ...
    function testRecordOvercommittedBeaconChainETHFailsWhenNotCalledByBionicPodManager(address improperCaller) public filterFuzzedAddressInputs(improperCaller) {
        uint256 amount = 1e18;
        address staker = address(this);
    ...
    function testCompleteQueuedWithdrawal_ReceiveAsTokensMarkedTrue_WithdrawingBeaconChainETH() external {
        ...
        require(sharesAfter == sharesBefore, "sharesAfter != sharesBefore");
        // require(balanceAfter == balanceBefore + withdrawalAmount, "balanceAfter != balanceBefore + withdrawalAmount");
        // TODO: make BionicPodManagerMock do something so we can verify that it gets called appropriately?
    }
...
testQueueWithdrawal_ToSelf_NotBeaconChainETHTwoStrategies(depositAmount, withdrawalAmount, undelegateIfPossible);
    ...
    function _beaconChainReentrancyTestsSetup() internal {
        // prepare StrategyManager with BionicPodManager and Delegation replaced with a Reenterer contract
        reenterer = new Reenterer();
        strategyManagerImplementation = new StrategyManager(IDelegationManager(address(reenterer)), IBionicPodManager(address(reenterer)), slasherMock);
        strategyManager = StrategyManager(
            address(
                new TransparentUpgradeableProxy(
                    address(strategyManagerImplementation),
