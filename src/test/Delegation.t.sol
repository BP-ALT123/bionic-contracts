...
import "../test/BionicTestHelper.t.sol";

import "./mocks/MiddlewareRegistryMock.sol";
import "./mocks/MiddlewareVoteWeigherMock.sol";
import "./mocks/ServiceManagerMock.sol";

contract DelegationTests is BionicTestHelper {
    ...
    modifier fuzzedAmounts(uint256 ethAmount, uint256 bionicAmount) {
        cheats.assume(ethAmount >= 0 && ethAmount <= 1e18);
        cheats.assume(bionicAmount >= 0 && bionicAmount <= 1e18);
        _;
    }

    function setUp() public virtual override {
        BionicDeployer.setUp();
    ...
    function initializeMiddlewares() public {
...

        {
            ...
            // split 60% ETH quorum, 40% BIONIC quorum
            _quorumBips[0] = 6000;
            _quorumBips[1] = 4000;
            ...
            VoteWeigherBaseStorage.StrategyAndWeightingMultiplier[] memory bionicStratsAndMultipliers =
                new VoteWeigherBaseStorage.StrategyAndWeightingMultiplier[](1);
            bionicStratsAndMultipliers[0].strategy = bionicStrat;
            bionicStratsAndMultipliers[0].multiplier = multiplier;

            cheats.startPrank(BionicProxyAdmin.owner());
            BionicProxyAdmin.upgradeAndCall(
                TransparentUpgradeableProxy(payable(address(voteWeigher))),
                address(voteWeigherImplementation),
                abi.encodeWithSelector(MiddlewareVoteWeigherMock.initialize.selector, _quorumBips, ethStratsAndMultipliers, bionicStratsAndMultipliers) 
            );
            cheats.stopPrank();
    ...
    function testSelfOperatorDelegate(address sender) public {
        ...
        cheats.assume(sender != address(BionicProxyAdmin));
    ...
    function testDelegation(address operator, address staker, uint96 ethAmount, uint96 bionicAmount)
        public
        fuzzedAddress(operator)
        fuzzedAddress(staker)
        fuzzedAmounts(ethAmount, bionicAmount)
    {
        cheats.assume(staker != operator);
        // base strategy will revert if these amounts are too small on first deposit
        cheats.assume(ethAmount >= 1);
        cheats.assume(bionicAmount >= 1);
        
        _testDelegation(operator, staker, ethAmount, bionicAmount, voteWeigher);
    }

    /// @notice tests that a when an operator is delegated to, that delegation is properly accounted for.
    function testDelegationReceived(address _operator, address staker, uint64 ethAmount, uint64 bionicAmount)
        public
        ...
        fuzzedAmounts(ethAmount, bionicAmount)
    {
        ...
        _testDepositBionic(staker, bionicAmount);
...

        {
            ...
            uint256 stakerBionicWeight = strategyManager.stakerStrategyShares(staker, updatedStrategies[1]);

            uint256 operatorEthWeightAfter = voteWeigher.weightOfOperator(operator, 0);
            uint256 operatorBionicWeightAfter = voteWeigher.weightOfOperator(operator, 1);
            ...
            assertTrue(
                operatorBionicWeightAfter - amountsBefore[1] == stakerBionicWeight,
                "Bionic weights did not increment by the right amount"
            );
        }
        {
    ...
    function testUndelegation(address operator, address staker, uint96 ethAmount, uint96 bionicAmount)
        public
        fuzzedAddress(operator)
        fuzzedAddress(staker)
        fuzzedAmounts(ethAmount, bionicAmount)
    ...
    function testDelegateToBySignature(address operator, uint96 ethAmount, uint96 bionicAmount, uint256 expiry)
        public
        fuzzedAddress(operator)
    {
        address staker = cheats.addr(PRIVATE_KEY);
        _registerOperatorAndDepositFromStaker(operator, staker, ethAmount, bionicAmount); 
    ...
    function testDelegateToBySignature_WithContractWallet_Successfully(address operator, uint96 ethAmount, uint96 bionicAmount)
        public
        fuzzedAddress(operator)
    {
        address staker = cheats.addr(PRIVATE_KEY);
    ...
    function testDelegateToBySignature_WithContractWallet_BadSignature(address operator, uint96 ethAmount, uint96 bionicAmount)
        public
        fuzzedAddress(operator)
    {
        address staker = cheats.addr(PRIVATE_KEY);
    ...
    function testDelegateToBySignature_WithContractWallet_NonconformingWallet(address operator, uint96 ethAmount, uint96 bionicAmount, uint8 v, bytes32 r, bytes32 s)
        public
        fuzzedAddress(operator)
    {
        address staker = cheats.addr(PRIVATE_KEY);
    ...
    /// @notice tests delegation to Bionic via an ECDSA signatures with invalid signature
    /// @param operator is the operator being delegated to.
        ...
        fuzzedAddress(staker)
    {
        ...
        uint96 operatorBionicWeightBefore = voteWeigher.weightOfOperator(operator, 1);
        IDelegationManager.OperatorDetails memory operatorDetails = IDelegationManager.OperatorDetails({
            earningsReceiver: operator,
            delegationApprover: address(0),
            stakerOptOutWindowBlocks: 0
        ...
        _testDepositBionic(staker, 1e18);
        _testDelegateToOperator(staker, operator);
        uint96 operatorEthWeightAfter = voteWeigher.weightOfOperator(operator, 0);
        uint96 operatorBionicWeightAfter = voteWeigher.weightOfOperator(operator, 1);
        assertTrue(
            operatorEthWeightAfter > operatorEthWeightBefore, "testDelegation: operatorEthWeight did not increase!"
        );
        assertTrue(
            operatorBionicWeightAfter > operatorBionicWeightBefore, "testDelegation: operatorEthWeight did not increase!"
        );
    }
    ...
    function testCannotInitMultipleTimesDelegation() public cannotReinit {
        ...
        delegation.initialize(address(this), BionicPauserReg, 0);
    }
    ...
    function testDelegationToUnregisteredDelegate(address delegate) public fuzzedAddress(delegate) {
        ...
        _testDepositBionic(getOperatorAddress(1), 1e18);

        cheats.expectRevert(bytes("DelegationManager._delegate: operator is not registered in Bionic"));
    ...
    function testCannotInitMultipleTimesDelegation(address _attacker) public {
        cheats.assume(_attacker != address(BionicProxyAdmin));
        //delegation has already been initialized in the Deployer test contract
        vm.prank(_attacker);
        cheats.expectRevert(bytes("Initializable: contract is already initialized"));
        delegation.initialize(_attacker, BionicPauserReg, 0);
    }
    ...
    function testUndelegate_SigP_Version(address _operator,address _staker,address _dt) public {
        ...
        vm.assume(_dt != address(0));
        vm.assume(_operator != address(BionicProxyAdmin));
        vm.assume(_staker != address(BionicProxyAdmin));
    ...
    function _testRegisterAdditionalOperator(uint256 index, uint32 _serveUntil) internal {
        ...
        //register as both ETH and BIONIC operator
        uint256 wethToDeposit = 1e18;
        uint256 bionicToDeposit = 1e10;
        _testDepositWeth(sender, wethToDeposit);
        _testDepositBionic(sender, bionicToDeposit);
        IDelegationManager.OperatorDetails memory operatorDetails = IDelegationManager.OperatorDetails({
            earningsReceiver: sender,
            delegationApprover: address(0),
            stakerOptOutWindowBlocks: 0
    ...
    // registers the operator if they are not already registered, and deposits "WETH" + "BIONIC" on behalf of the staker.
    function _registerOperatorAndDepositFromStaker(address operator, address staker, uint96 ethAmount, uint96 bionicAmount) internal {
        cheats.assume(staker != operator);
        ...
        cheats.assume(bionicAmount >= 1 && bionicAmount <= 1e18);
