...
contract BionicPodTests is ProofParsing, BionicPodPausingConstants {
    ...
    IBionicPodManager public bionicPodManager;
    IBionicPod public podImplementation;
    ...
    // BIONICPOD EVENTS
    /// @notice Emitted when an ETH validator stakes via this bionicPod
    event BionicPodStaked(bytes pubkey);

    /// @notice Emitted when an ETH validator's withdrawal credentials are successfully verified to be pointed to this bionicPod
    event ValidatorRestaked(uint40 validatorIndex);

    /// @notice Emitted when an ETH validator's balance is updated in Bionic
    ...
    /// @notice Emitted when restaked beacon chain ETH is withdrawn from the bionicPod.
    ...
    function setUp() public {
        ...
        podImplementation = new BionicPod(
                ethPOSDeposit, 
                delayedWithdrawalRouter,
                IBionicPodManager(podManagerAddress),
                MAX_VALIDATOR_BALANCE_GWEI,
                EFFECTIVE_RESTAKED_BALANCE_OFFSET
        );
        bionicPodBeacon = new UpgradeableBeacon(address(podImplementation));
        ...
        StrategyManager strategyManagerImplementation = new StrategyManager(delegation, IBionicPodManager(podManagerAddress), slasher);
        ...
        //ensuring that the address of bionicpodmanager doesn't change
        bytes memory code = address(bionicPodManager).code;
        cheats.etch(podManagerAddress, code);
        bionicPodManager = IBionicPodManager(podManagerAddress);

        beaconChainOracle = new BeaconChainOracleMock();
        DelayedWithdrawalRouter delayedWithdrawalRouterImplementation = new DelayedWithdrawalRouter(IBionicPodManager(podManagerAddress));
    ...
    function testStaking() public {
        ...
        IBionicPod newPod = bionicPodManager.getPod(podOwner);
        cheats.expectEmit(true, true, true, true, address(newPod));
        emit BionicPodStaked(pubkey);
        bionicPodManager.stake{value: stakeAmount}(pubkey, signature, depositDataRoot);
        cheats.stopPrank();
    }

    function testWithdrawBeforeRestaking() public {
        testStaking();
        IBionicPod pod = bionicPodManager.getPod(podOwner);
    ...
    function testDeployBionicPodWithoutActivateRestaking() public {
        ...
        cheats.expectRevert(bytes("BionicPod.hasEnabledRestaking: restaking is not enabled"));
        newPod.verifyWithdrawalCredentials(timestamp, validatorIndices, proofsArray, validatorFieldsArray);
        cheats.stopPrank();
    }

    function testDeployBionicPodTooSoon() public {
        // ./solidityProofGen "ValidatorFieldsProof" 61336 true "data/slot_58000/oracle_capella_beacon_state_58100.ssz" "withdrawalCredentialAndBalanceProof_61336.json"
        setJSON("./src/test/test-data/withdrawalCredentialAndBalanceProof_61336.json");
        ...
        cheats.startPrank(podOwner);
        cheats.expectRevert(bytes("BionicPod.proofIsForValidTimestamp: beacon chain proof must be for timestamp after mostRecentWithdrawalTimestamp"));
        newPod.verifyWithdrawalCredentials(timestamp, validatorIndices, proofsArray, validatorFieldsArray);
        cheats.stopPrank();
    }
    ...
    function testWithdrawBeforeRestakingAfterRestaking() public {
        // ./solidityProofGen "ValidatorFieldsProof" 61336 true "data/slot_58000/oracle_capella_beacon_state_58100.ssz" "withdrawalCredentialAndBalanceProof_61336.json"
        setJSON("./src/test/test-data/withdrawalCredentialAndBalanceProof_61336.json");
        IBionicPod pod = _testDeployAndVerifyNewBionicPod(podOwner, signature, depositDataRoot);

        cheats.expectRevert(bytes("BionicPod.hasNeverRestaked: restaking is enabled"));
        cheats.startPrank(podOwner);
        pod.withdrawBeforeRestaking();
        cheats.stopPrank();
    }
    ...
    function testAttemptedWithdrawalAfterVerifyingWithdrawalCredentials() public {
        testDeployAndVerifyNewBionicPod();
        IBionicPod pod = bionicPodManager.getPod(podOwner);
        cheats.startPrank(podOwner);
        cheats.expectRevert(bytes("BionicPod.hasNeverRestaked: restaking is enabled"));
        IBionicPod(pod).withdrawBeforeRestaking();
        cheats.stopPrank();
    }
    ...
    function testFullWithdrawalFlow() public returns (IBionicPod) {
        //this call is to ensure that validator 61336 has proven their withdrawalcreds
        // ./solidityProofGen "ValidatorFieldsProof" 61336 true "data/slot_58000/oracle_capella_beacon_state_58100.ssz" "withdrawalCredentialAndBalanceProof_61336.json"
        setJSON("./src/test/test-data/withdrawalCredentialAndBalanceProof_61336.json");
        _testDeployAndVerifyNewBionicPod(podOwner, signature, depositDataRoot);
        IBionicPod newPod = bionicPodManager.getPod(podOwner);
    ...
    function testPartialWithdrawalFlow() public returns(IBionicPod) {
    ...
    function testProvingMultiplePartialWithdrawalsForSameSlot(/*uint256 numPartialWithdrawals*/) public {
        IBionicPod newPod = testPartialWithdrawalFlow();
        ...
        cheats.expectRevert(bytes("BionicPod._verifyAndProcessWithdrawal: withdrawal has already been proven for this slot"));
        newPod.verifyAndProcessWithdrawals(withdrawalProofsArray, validatorFieldsProofArray, validatorFieldsArray, withdrawalFieldsArray, 0, 0);
    }

    /// @notice verifies that multiple full withdrawals for a single validator fail
    function testDoubleFullWithdrawal() public returns(IBionicPod newPod) {
    ...
    function testDeployAndVerifyNewBionicPod() public returns(IBionicPod) {
        ...
        setJSON("./src/test/test-data/withdrawalCredentialAndBalanceProof_61068.json");
        return _testDeployAndVerifyNewBionicPod(podOwner, signature, depositDataRoot);
    }
    ...
    //test deploying an bionic pod with mismatched withdrawal credentials between the proof and the actual pod's address
    function testDeployNewBionicPodWithWrongWithdrawalCreds(address wrongWithdrawalAddress) public {
        setJSON("./src/test/test-data/withdrawalCredentialAndBalanceProof_61068.json");
        cheats.startPrank(podOwner);
        bionicPodManager.stake{value: stakeAmount}(pubkey, signature, depositDataRoot);
        cheats.stopPrank();
        ...
        IBionicPod newPod;
        newPod = bionicPodManager.getPod(podOwner);
        ...
        cheats.warp(timestamp += 1);
        cheats.expectRevert(bytes("BionicPod.verifyCorrectWithdrawalCredentials: Proof is not for this BionicPod"));
        newPod.verifyWithdrawalCredentials(timestamp, validatorIndices, proofsArray, validatorFieldsArray);
        cheats.stopPrank();
    }
    ...
    function testVerifyWithdrawalCredsFromNonPodOwnerAddress(address nonPodOwnerAddress) public {
        ...
        cheats.expectRevert(bytes("BionicPod.onlyBionicPodOwner: not podOwner"));
        newPod.verifyWithdrawalCredentials(timestamp, validatorIndices, proofsArray, validatorFieldsArray);
        cheats.stopPrank();
    }

    //test that when withdrawal credentials are verified more than once, it reverts
    function testDeployNewBionicPodWithActiveValidator() public {
        // ./solidityProofGen "ValidatorFieldsProof" 61068 false "data/slot_58000/oracle_capella_beacon_state_58100.ssz" "withdrawalCredentialAndBalanceProof_61068.json"
        setJSON("./src/test/test-data/withdrawalCredentialAndBalanceProof_61068.json");
        IBionicPod pod = _testDeployAndVerifyNewBionicPod(podOwner, signature, depositDataRoot);
        ...
        cheats.startPrank(podOwner);
        cheats.expectRevert(bytes("BionicPod.verifyCorrectWithdrawalCredentials: Validator must be inactive to prove withdrawal credentials"));
    ...
    function testProveSingleWithdrawalCredential() public {
        ...
        assertTrue(pod.validatorStatus(validatorPubkeyHash) == IBionicPod.VALIDATOR_STATUS.ACTIVE, "wrong validator status");
    }
    ...
    function testProveOverCommittedBalance() public {
        ...
        setJSON("./src/test/test-data/slashedProofs/balanceUpdateProof_notOvercommitted_61511.json");
        IBionicPod newPod = _testDeployAndVerifyNewBionicPod(podOwner, signature, depositDataRoot);
        // get beaconChainETH shares
        uint256 beaconChainETHBefore = getBeaconChainETHShares(podOwner);
    ...
    function testTooSoonBalanceUpdates() public {
        ...
        cheats.expectRevert(bytes("BionicPod.verifyBalanceUpdate: Validator's balance has already been updated for this slot"));
        newPod.verifyBalanceUpdate(validatorIndex, proofs, validatorFields, 0, uint64(block.number));
    }

    function testDeployingBionicPodRevertsWhenPaused() external {
        // pause the contract
        cheats.startPrank(pauser);
        bionicPodManager.pause(2 ** PAUSED_NEW_BIONICPODS);
        cheats.stopPrank();
    ...
    function testCreatePodWhenPaused() external {
        ...
        cheats.startPrank(podOwner);
        cheats.expectRevert(bytes("Pausable: index is paused"));
        bionicPodManager.createPod();
        cheats.stopPrank();
    }

    function testStakeOnBionicPodFromNonPodManagerAddress(address nonPodManager) external fuzzedAddress(nonPodManager) {
        cheats.assume(nonPodManager != address(bionicPodManager));
        ...
        cheats.expectRevert(bytes("BionicPod.onlyBionicPodManager: not bionicPodManager"));
    ...
    function testWithdrawRestakedBeaconChainETHRevertsWhenPaused() external {
        // pause the contract
        cheats.startPrank(pauser);
        bionicPodManager.pause(2 ** PAUSED_WITHDRAW_RESTAKED_ETH);
        cheats.stopPrank();

        address recipient = address(this);
        uint256 amount = 1e18;
        cheats.startPrank(address(bionicPodManager.strategyManager()));
        cheats.expectRevert(bytes("Pausable: index is paused"));
        bionicPodManager.withdrawRestakedBeaconChainETH(podOwner, recipient, amount);
        cheats.stopPrank();
    }
    ...
    function testVerifyCorrectWithdrawalCredentialsRevertsWhenPaused() external {
        ...
        // pause the contract
        cheats.startPrank(pauser);
        bionicPodManager.pause(2 ** PAUSED_BIONICPODS_VERIFY_CREDENTIALS);
        cheats.stopPrank();
        ...
        cheats.startPrank(podOwner);
        cheats.expectRevert(bytes("BionicPod.onlyWhenNotPaused: index is paused in BionicPodManager"));
        newPod.verifyWithdrawalCredentials(timestamp, validatorIndices, proofsArray, validatorFieldsArray);
        cheats.stopPrank();
    }
    ...
    function testVerifyOvercommittedStakeRevertsWhenPaused() external {
        ...
        // pause the contract
        cheats.startPrank(pauser);
        bionicPodManager.pause(2 ** PAUSED_BIONICPODS_VERIFY_BALANCE_UPDATE);
        cheats.stopPrank();
    ...
    function _proveOverCommittedStake(IBionicPod newPod) internal {
        validatorFields = getValidatorFields();
        uint40 validatorIndex = uint40(getValidatorIndex());
    ...
    function _proveUnderCommittedStake(IBionicPod newPod) internal {
        validatorFields = getValidatorFields();
        uint40 validatorIndex = uint40(getValidatorIndex());
        bytes32 newLatestBlockHeaderRoot = getLatestBlockHeaderRoot();
        BeaconChainOracleMock(address(beaconChainOracle)).setBeaconChainStateRoot(newLatestBlockHeaderRoot);
        ...
        emit ValidatorBalanceUpdated(validatorIndex, _getEffectiveRestakedBalanceGwei(Endian.fromLittleEndianUint64(validatorFields[BeaconChainProofs.VALIDATOR_BALANCE_INDEX])));
        newPod.verifyBalanceUpdate(validatorIndex, proofs, validatorFields, 0, uint64(block.number));
        require(newPod.validatorPubkeyHashToInfo(getValidatorPubkeyHash()).status == IBionicPod.VALIDATOR_STATUS.ACTIVE);
    }
    ...
    function testStake(bytes calldata _pubkey, bytes calldata _signature, bytes32 _depositDataRoot) public {
        ...
        cheats.startPrank(podOwner);
        cheats.expectRevert("BionicPod.stake: must initially stake for any validator with 32 ether");
        bionicPodManager.stake(_pubkey, _signature, _depositDataRoot);
        cheats.expectRevert("BionicPod.stake: must initially stake for any validator with 32 ether");
        bionicPodManager.stake{value: 12 ether}(_pubkey, _signature, _depositDataRoot);

        IBionicPod newPod = bionicPodManager.getPod(podOwner);

        // successful call
        cheats.expectEmit(true, true, true, true, address(newPod));
        emit BionicPodStaked(_pubkey);
        bionicPodManager.stake{value: 32 ether}(_pubkey, _signature, _depositDataRoot);
        cheats.stopPrank();
    }
    ...
    // verifies that the `numPod` variable increments correctly on a succesful call to the `BionicPod.stake` function
    function test_incrementNumPodsOnStake(bytes calldata _pubkey, bytes calldata _signature, bytes32 _depositDataRoot) public {
        uint256 numPodsBefore = BionicPodManager(address(bionicPodManager)).numPods();
        testStake(_pubkey, _signature, _depositDataRoot);
        uint256 numPodsAfter = BionicPodManager(address(bionicPodManager)).numPods();
        require(numPodsAfter == numPodsBefore + 1, "numPods did not increment correctly");
    }

    // verifies that the `maxPods` variable is enforced on the `BionicPod.stake` function
    function test_maxPodsEnforcementOnStake(bytes calldata _pubkey, bytes calldata _signature, bytes32 _depositDataRoot) public {
        ...
        BionicPodManager(address(bionicPodManager)).setMaxPods(BionicPodManager(address(bionicPodManager)).numPods());
        cheats.stopPrank();

        cheats.startPrank(podOwner);
        cheats.expectRevert("BionicPodManager._deployPod: pod limit reached");
        bionicPodManager.stake{value: 32 ether}(_pubkey, _signature, _depositDataRoot);
        cheats.stopPrank();

        // set pod limit to *one more than* current number of pods
        cheats.startPrank(unpauser);
        BionicPodManager(address(bionicPodManager)).setMaxPods(BionicPodManager(address(bionicPodManager)).numPods() + 1);
        cheats.stopPrank();
    ...
    // verifies that the `numPod` variable increments correctly on a succesful call to the `BionicPod.createPod` function
    ...
    function test_createPodTwiceFails() public {
        bionicPodManager.createPod();
        cheats.expectRevert(bytes("BionicPodManager.createPod: Sender already has a pod"));
        bionicPodManager.createPod();
    }

    // verifies that the `maxPods` variable is enforced on the `BionicPod.createPod` function
    function test_maxPodsEnforcementOnCreatePod() public {
        // set pod limit to current number of pods
        cheats.startPrank(unpauser);
        uint256 previousValue = BionicPodManager(address(bionicPodManager)).maxPods();
        uint256 newValue = BionicPodManager(address(bionicPodManager)).numPods();
        cheats.expectEmit(true, true, true, true, address(bionicPodManager));
        emit MaxPodsUpdated(previousValue, newValue);
        BionicPodManager(address(bionicPodManager)).setMaxPods(newValue);
        cheats.stopPrank();
        ...
        // set pod limit to *one more than* current number of pods
        cheats.startPrank(unpauser);
        previousValue = BionicPodManager(address(bionicPodManager)).maxPods();
        newValue = BionicPodManager(address(bionicPodManager)).numPods() + 1;
        cheats.expectEmit(true, true, true, true, address(bionicPodManager));
        emit MaxPodsUpdated(previousValue, newValue);
        BionicPodManager(address(bionicPodManager)).setMaxPods(newValue);
        cheats.stopPrank();
    ...
    function test_setMaxPods(uint256 newValue) public {
        ...
        require(BionicPodManager(address(bionicPodManager)).maxPods() == newValue, "maxPods value not set correctly");
    }
    ...
    function testQueueBeaconChainETHWithdrawalWithoutProvingFullWithdrawal() external {
        ...
        cheats.expectRevert("BionicPod.decrementWithdrawableRestakedExecutionLayerGwei: amount to decrement is greater than current withdrawableRestakedRxecutionLayerGwei balance");
    ...
    function testQueueBeaconChainETHWithdrawal() external {
        IBionicPod pod = testFullWithdrawalFlow();
        ...
        bool undelegateIfPossible = false;
        _verifyBionicPodBalanceSharesInvariant(podOwner, pod, validatorPubkeyHash);
        _testQueueWithdrawal(podOwner, strategyIndexes, strategyArray, shareAmounts, undelegateIfPossible);
        _verifyBionicPodBalanceSharesInvariant(podOwner, pod, validatorPubkeyHash);
    ...
    function _verifyBionicPodBalanceSharesInvariant(address podowner, IBionicPod pod, bytes32 validatorPubkeyHash) internal {
        ...
        BionicPod.ValidatorInfo memory info = pod.validatorPubkeyHashToInfo(validatorPubkeyHash);
        ...
        require(sharesInSM/GWEI_TO_WEI == validatorBalanceGwei + withdrawableRestakedExecutionLayerGwei, "BionicPod invariant violated: sharesInSM != withdrawableRestakedExecutionLayerGwei");
    }
    ...
    function _testDeployAndVerifyNewBionicPod(address _podOwner, bytes memory _signature, bytes32 _depositDataRoot)
        internal returns (IBionicPod)
    {
        ...
        IBionicPod newPod = bionicPodManager.getPod(_podOwner);

        cheats.startPrank(_podOwner);
        cheats.expectEmit(true, true, true, true, address(newPod));
        emit BionicPodStaked(pubkey);
        bionicPodManager.stake{value: stakeAmount}(pubkey, _signature, _depositDataRoot);
        cheats.stopPrank();
