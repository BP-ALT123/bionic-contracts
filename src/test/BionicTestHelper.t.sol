...
import "../test/BionicDeployer.t.sol";


contract BionicTestHelper is BionicDeployer {
     ...
     * into both Weth and Bionic strategies, as well as delegating assets from "stakers" to the operator.
     * @param operatorIndex is the index of the operator to use from the test-data/operators.json file
     * @param amountBionicToDeposit amount of bionic token to deposit
     * @param amountEthToDeposit amount of eth to deposit
     */

    function _testInitiateDelegation(
        uint8 operatorIndex,
        uint256 amountBionicToDeposit, 
        uint256 amountEthToDeposit        
    )
        public returns (uint256 amountEthStaked, uint256 amountBionicStaked)
    {
        ...
        for (uint256 i; i < stakers.length; i++) {
            //initialize weth, bionic and eth balances for staker
            bionicToken.transfer(stakers[i], amountBionicToDeposit);
            weth.transfer(stakers[i], amountEthToDeposit);

            //deposit staker's bionic and weth into strategy manager
            _testDepositBionic(stakers[i], amountBionicToDeposit);
            _testDepositWeth(stakers[i], amountEthToDeposit);

            //delegate the staker's deposits to operator
            uint256 operatorBionicSharesBefore = delegation.operatorShares(operator, bionicStrat);
            ...
            assertTrue(
                delegation.operatorShares(operator, bionicStrat) - operatorBionicSharesBefore == amountBionicToDeposit
            );
        ...
        amountBionicStaked += delegation.operatorShares(operator, bionicStrat);

        return (amountEthStaked, amountBionicStaked);
    }
    ...
    /**
     * @notice Deposits `amountToDeposit` of BIONIC from address `sender` into `bionicStrat`.
     * @param sender The address to spoof calls from using `cheats.startPrank(sender)`
     * @param amountToDeposit Amount of BIONIC that is first *transferred from this contract to `sender`* and then deposited by `sender` into `stratToDepositTo`
     */
    function _testDepositBionic(address sender, uint256 amountToDeposit) internal returns (uint256 amountDeposited) {
        cheats.assume(amountToDeposit <= bionicTotalSupply);
        amountDeposited = _testDepositToStrategy(sender, amountToDeposit, bionicToken, bionicStrat);
    }
        ...
        for (uint8 i = 0; i < numStratsToAdd; ++i) {
            stratsToDepositTo[i] = StrategyBase(
                address(
                    ...
                    abi.encodeWithSelector(StrategyBase.initialize.selector, underlyingToken, BionicPauserReg)
                    )
                )
            );
    ...
    */
    ...
    function _testDelegation(address operator, address staker, uint256 ethAmount, uint256 bionicAmount, IVoteWeigher voteWeigher)
        internal
    {
