...
contract DepositWithdrawTests is BionicTestHelper {
    ...
    /// @notice Verifies that it is possible to deposit bionic.
    /// @param bionicToDeposit is amount of bionic to deposit into the bionic strategy
    function testDepositBionic(uint96 bionicToDeposit) public {
        // sanity check for inputs; keeps fuzzed tests from failing
        cheats.assume(bionicToDeposit < bionicTotalSupply);
        // if first deposit amount to base strategy is too small, it will revert. ignore that case here.
        cheats.assume(bionicToDeposit >= 1);
        _testDepositBionic(getOperatorAddress(0), bionicToDeposit);
    }
