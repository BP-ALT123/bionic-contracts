...
/**
 * @title The primary entry- and exit-point for funds into and out of Bionic.
    ...
    StrategyManagerStorage
{
     ...
     * @param depositor Is the staker who is depositing funds into Bionic.
     ...
     * @param depositor Is the staker who is queuing a withdrawal from Bionic.
     ...
     * @param depositor Is the staker who is withdrawing funds from Bionic.
     * @param nonce Is the withdrawal's unique identifier (to the depositor).
    ...
    modifier onlyBionicPodManager {
        require(address(bionicPodManager) == msg.sender, "StrategyManager.onlyBionicPodManager: not the bionicPodManager");
        _;
    }
    ...
    /**
     * @param _delegation The delegation contract of Bionic.
     * @param _slasher The primary slashing contract of Bionic.
     * @param _bionicPodManager The contract that keeps track of BionicPod stakes for restaking beacon chain ether.
     */
    constructor(IDelegationManager _delegation, IBionicPodManager _bionicPodManager, ISlasher _slasher)
        StrategyManagerStorage(_delegation, _bionicPodManager, _slasher)
    {
     ...
     * @param staker is the entity that is restaking in Bionic,
     * @param amount is the amount of beaconchain ETH being restaked,
     * @dev Only callable by BionicPodManager.
     */
    ...
    /**
     * @notice Called by a staker to undelegate entirely from Bionic. The staker must first withdraw all of their existing deposits
     * (through use of the `queueWithdrawal` function), or else otherwise have never deposited in Bionic prior to delegating.
     */
     ...
     * This function queues a withdrawal of all of the `staker`'s shares in Bionic to the staker themself, and then undelegates the staker.
     ...
     * the enshrined 'beaconChainETH' strategy technically represent non-fungible positions (deposits to the Beacon Chain, each pointed at a specific BionicPod).
     */
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < strategiesLength;) {
            ...
            if (strategies[i] == beaconChainETHStrategy) {
                ...
                bionicPodManager.withdrawRestakedBeaconChainETH(slashedAddress, recipient, shareAmounts[i]);
            }
            else {
        ...
        for (uint256 i = 0; i < strategiesLength;) {
            ...
            if (indicesToSkipIndex < indicesToSkip.length && indicesToSkip[indicesToSkipIndex] == i) {
                ...
                if (queuedWithdrawal.strategies[i] == beaconChainETHStrategy){
                     //withdraw the beaconChainETH to the recipient
                    bionicPodManager.withdrawRestakedBeaconChainETH(queuedWithdrawal.depositor, recipient, queuedWithdrawal.shares[i]);
                } else {
    ...
    */
        ...
        returns (bytes32)
    {
         ...
         * This is because shares in the enshrined `beaconChainETHStrategy` ultimately represent tokens in **non-fungible** BionicPods,
        ...
        for (uint256 i = 0; i < strategies.length;) {
            if (strategies[i] == beaconChainETHStrategy) {
                ...
                * Remember that withdrawableRestakedExecutionLayerGwei tracks the currently withdrawable ETH from the BionicPod.  
                * By doing this, we ensure that the number of shares in Bionic matches the amount of withdrawable ETH in 
                ...
                * via Bionic, since otherwise withdrawableRestakedExecutionLayerGwei will be 0.
                */         
                bionicPodManager.decrementWithdrawableRestakedExecutionLayerGwei(msg.sender, shares[i]);   
        ...
        // If the `staker` has withdrawn all of their funds from Bionic in this transaction, then they can choose to also undelegate
        /**
         ...
         * in the case that 'undelegate' is set to true but the `staker` still has active deposits in Bionic.
         */
        ...
        internal onlyNotFrozen(queuedWithdrawal.delegatedAddress)
    {
        ...
        // enforce minimum delay lag (not applied to withdrawals of 'beaconChainETH', since the BionicPods enforce their own delay)
        ...
        if (receiveAsTokens) {
            ...
            for (uint256 i = 0; i < strategiesLength;) {
                if (queuedWithdrawal.strategies[i] == beaconChainETHStrategy) {
                    ...
                    // if the strategy is the beaconchaineth strategy, then withdraw through the ETH from the BionicPod
                    bionicPodManager.withdrawRestakedBeaconChainETH(queuedWithdrawal.depositor, msg.sender, queuedWithdrawal.shares[i]);
                } else {
            ...
            for (uint256 i = 0; i < strategiesLength;) {
                ...
                if(queuedWithdrawal.strategies[i] == beaconChainETHStrategy) {
                    ...
                    bionicPodManager.incrementWithdrawableRestakedExecutionLayerGwei(queuedWithdrawal.depositor, queuedWithdrawal.shares[i]);
                }
                unchecked {
                    ++i;
                }
     ...
     * This allows people a "hard reset" in their relationship with Bionic after withdrawing all of their stake.
    ...
    function _calculateDomainSeparator() internal view returns (bytes32) {
        return keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("Bionic")), block.chainid, address(this)));
    }
}
