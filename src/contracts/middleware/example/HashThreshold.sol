...
/**
 * @title An Bionic middleware example service manager that slashes validators that sign a message that, when hashed 10 times starts with less than a certain number of 0s.
...
contract HashThreshold is Ownable, IServiceManager {
    ...
    function slashSigners(bytes32 message, bytes calldata signatures) external {
        ...
        for (uint i = 0; i < signatures.length; i += 65) {
            // this is Bionic's means of escalating an operators stake for review for slashing
