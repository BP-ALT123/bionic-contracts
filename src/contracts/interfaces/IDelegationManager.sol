 ...
 * @notice  This is the contract for delegation in Bionic. The main functionalities of this contract are
 * - enabling anyone to register as an operator in Bionic
 * - allowing operators to specify parameters related to stakers who delegate to them
 * - enabling any staker to delegate its stake to the operator of its choice (a given staker can only delegate to a single operator at a time)
 * - enabling a staker to undelegate its assets from the operator it is delegated to (performed as part of the withdrawal process, initiated through the StrategyManager)
 */
interface IDelegationManager {
    // @notice Struct used for storing information about a single operator who has registered with Bionic
    struct OperatorDetails {
        // @notice address to receive the rewards that the operator earns via serving applications built on Bionic.
    ...
    /**
     * @notice Registers the caller as an operator in Bionic.
     ...
     * 
     * @dev The caller must have previously registered as an operator in Bionic.
     * @dev This function will revert if the caller attempts to set their `earningsReceiver` to address(0).
     */
     ...
     * @param operator The account (`msg.sender`) is delegating its assets to for use in serving applications built on Bionic.
     ...
     * @param operator The account (`staker`) is delegating its assets to for use in serving applications built on Bionic.
     ...
     * @dev Should only ever be called in the event that the `staker` has no active deposits in Bionic.
