...
contract DelayedWithdrawalRouter is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, Pausable, IDelayedWithdrawalRouter {
    ...
    /// @notice The BionicPodManager contract of Bionic.
    ...
    /// @notice Modifier used to permission a function to only be called by the BionicPod of the specified `podOwner`
    modifier onlyBionicPod(address podOwner) {
        require(address(bionicPodManager.getPod(podOwner)) == msg.sender, "DelayedWithdrawalRouter.onlyBionicPod: not podOwner's BionicPod");
        _;
    }

    constructor(IBionicPodManager _bionicPodManager) {
        require(address(_bionicPodManager) != address(0), "DelayedWithdrawalRouter.constructor: _bionicPodManager cannot be zero address");
     ...
     * @dev Only callable by the `podOwner`'s BionicPod contract.
     */
    function createDelayedWithdrawal(address podOwner, address recipient) external payable onlyBionicPod(podOwner) onlyWhenNotPaused(PAUSED_DELAYED_WITHDRAWAL_CLAIMS) {
