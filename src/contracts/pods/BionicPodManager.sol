 ...
 */
contract BionicPodManager is Initializable, OwnableUpgradeable, Pausable, IBionicPodManager, BionicPodPausingConstants {
    /**
     ...
     * addresses of BionicPods that are pre-computed with Create2 to change, even upon upgrading this contract, changing compiler version, etc.
    */
    ...
    /// @notice Pod owner to deployed BionicPod address
    mapping(address => IBionicPod) public ownerToPod;

    // BEGIN STORAGE VARIABLES ADDED AFTER FIRST TESTNET DEPLOYMENT -- DO NOT SUGGEST REORDERING TO CONVENTIONAL ORDER
    /// @notice The number of BionicPods that have been deployed
    uint256 public numPods;

    /// @notice The maximum number of BionicPods that can be deployed
    ...
    modifier onlyBionicPod(address podOwner) {
        require(address(ownerToPod[podOwner]) == msg.sender, "BionicPodManager.onlyBionicPod: not a pod");
        _;
    }
    ...
    function stake(bytes calldata pubkey, bytes calldata signature, bytes32 depositDataRoot) external payable {
        IBionicPod pod = ownerToPod[msg.sender];
    ...
    // INTERNAL FUNCTIONS
    function _deployPod() internal onlyWhenNotPaused(PAUSED_NEW_BIONICPODS) returns (IBionicPod) {
        // check that the limit of BionicPods has not been hit, and increment the BionicPod count
        require(numPods + 1 <= maxPods, "BionicPodManager._deployPod: pod limit reached");
        ++numPods;
        // create the pod
        IBionicPod pod = 
            IBionicPod(
    ...
    function getPod(address podOwner) public view returns (IBionicPod) {
        IBionicPod pod = ownerToPod[podOwner];
    ...
    function getBeaconChainStateRoot(uint64 blockNumber) external view returns(bytes32) {
        ...
        require(stateRoot != bytes32(0), "BionicPodManager.getBeaconChainStateRoot: state root at blockNumber not yet finalized");
        return stateRoot;
    }
