...
import "../interfaces/IBionicPodManager.sol";
import "../interfaces/IBionicPod.sol";
import "../interfaces/IDelayedWithdrawalRouter.sol";
import "../interfaces/IPausable.sol";

import "./BionicPodPausingConstants.sol";

/**
 * @title The implementation contract used for restaking beacon chain ETH on Bionic 
 ...
 * - updating aggregate balances in the BionicPodManager
 * - withdrawing eth when withdrawals are initiated
 * @dev Note that all beacon chain balances are stored as gwei within the beacon chain datastructures. We choose
 *   to account balances in terms of gwei in the BionicPod contract and convert to wei when making calls to other contracts
 */
contract BionicPod is IBionicPod, Initializable, ReentrancyGuardUpgradeable, BionicPodPausingConstants {
    ...
    /// @notice The single BionicPodManager for Bionic
    IBionicPodManager public immutable bionicPodManager;
    ...
    */
    ...
    /// @notice The owner of this BionicPod
    ...
    // STORAGE VARIABLES
    /// @notice the amount of execution layer ETH in this contract that is staked in Bionic (i.e. withdrawn from the Beacon Chain but not from Bionic), 
    ...
    //  is the validator's balance that is credited on Bionic.
    ...
    modifier onlybionicPodManager {
        require(msg.sender == address(bionicPodManager), "BionicPod.onlybionicPodManager: not bionicPodManager");
        _;
    }

    modifier onlyBionicPodOwner {
        require(msg.sender == podOwner, "BionicPod.onlyBionicPodOwner: not podOwner");
        _;
    }

    modifier onlyNotFrozen {
        require(!bionicPodManager.slasher().isFrozen(podOwner), "BionicPod.onlyNotFrozen: pod owner is frozen");
        _;
    }

    modifier hasNeverRestaked {
        require(!hasRestaked, "BionicPod.hasNeverRestaked: restaking is enabled");
        _;
    }

    /// @notice checks that hasRestaked is set to true by calling activateRestaking()
    modifier hasEnabledRestaking {
        require(hasRestaked, "BionicPod.hasEnabledRestaking: restaking is not enabled");
        _;
    }
    ...
    modifier proofIsForValidTimestamp(uint64 timestamp) {
        require(timestamp > mostRecentWithdrawalTimestamp,
            "BionicPod.proofIsForValidTimestamp: beacon chain proof must be for timestamp after mostRecentWithdrawalTimestamp");
        _;
    }

    /**
     * @notice Based on 'Pausable' code, but uses the storage of the BionicPodManager instead of this contract. This construction
     * is necessary for enabling pausing all BionicPods at the same time (due to BionicPods being Beacon Proxies).
     * Modifier throws if the `indexed`th bit of `_paused` in the BionicPodManager is 1, i.e. if the `index`th pause switch is flipped.
     */
    modifier onlyWhenNotPaused(uint8 index) {
        require(!IPausable(address(bionicPodManager)).paused(index), "BionicPod.onlyWhenNotPaused: index is paused in BionicPodManager");
        _;
    }
    ...
    constructor(
        ...
        IBionicPodManager _bionicPodManager,
        uint64 _MAX_VALIDATOR_BALANCE_GWEI,
        uint64 _RESTAKED_BALANCE_OFFSET_GWEI
    ) {
        ethPOS = _ethPOS;
        delayedWithdrawalRouter = _delayedWithdrawalRouter;
        bionicPodManager = _bionicPodManager;
        MAX_VALIDATOR_BALANCE_GWEI = _MAX_VALIDATOR_BALANCE_GWEI;
        RESTAKED_BALANCE_OFFSET_GWEI = _RESTAKED_BALANCE_OFFSET_GWEI;
        // _disableInitializers();
    }

    /// @notice Used to initialize the pointers to addresses crucial to the pod's functionality. Called on construction by the BionicPodManager.
    function initialize(address _podOwner) external initializer {
        require(_podOwner != address(0), "BionicPod.initialize: podOwner cannot be zero address");
        podOwner = _podOwner;
    }

    /// @notice Called by BionicPodManager when the owner wants to create another ETH validator.
    function stake(bytes calldata pubkey, bytes calldata signature, bytes32 depositDataRoot) external payable onlybionicPodManager {
        // stake on ethpos
        require(msg.value == 32 ether, "BionicPod.stake: must initially stake for any validator with 32 ether");
     ...
     * root, marks the validator as 'active' in Bionic, and credits the restaked ETH in Bionic.
    ...
    function verifyWithdrawalCredentials(
        ...
        onlyBionicPodOwner
        onlyWhenNotPaused(PAUSED_BIONICPODS_VERIFY_CREDENTIALS)
        ...
        hasEnabledRestaking
    {
        ...
        require(oracleTimestamp + VERIFY_WITHDRAWAL_CREDENTIAL_WINDOW >= block.timestamp,
            "BionicPod.verifyWithdrawalCredentials: specified timestamp is too far in past");



        require((validatorIndices.length == proofs.length) && (proofs.length == validatorFields.length), "BionicPod.verifyWithdrawalCredentials: validatorIndices and proofs must be same length");
        ...
        bionicPodManager.restakeBeaconChainETH(podOwner, totalAmountToBeRestakedWei);
    }
    ...
    function verifyBalanceUpdate(
        ...
        uint64 oracleTimestamp
    ) external onlyWhenNotPaused(PAUSED_BIONICPODS_VERIFY_BALANCE_UPDATE) {
       // ensure that the timestamp being proven against is not "too stale", i.e. that the validator's balance *recently* changed.
        require(oracleTimestamp + VERIFY_BALANCE_UPDATE_WINDOW_SECONDS >= block.timestamp,
            "BionicPod.verifyBalanceUpdate: specified timestamp is too far in past");
        ...
        require(validatorInfo.status == VALIDATOR_STATUS.ACTIVE, "BionicPod.verifyBalanceUpdate: Validator not active");
        ...
        require(validatorInfo.mostRecentBalanceUpdateSlot < Endian.fromLittleEndianUint64(proofs.slotRoot),
            "BionicPod.verifyBalanceUpdate: Validator's balance has already been updated for this slot");
...

        {
            ...
            bytes32 latestBlockHeaderRoot = bionicPodManager.getBeaconChainStateRoot(oracleTimestamp);
        ...
        if (newRestakedBalanceGwei != currentRestakedBalanceGwei){
            ...
            bionicPodManager.recordBeaconChainETHBalanceUpdate(podOwner, beaconChainETHStrategyIndex, sharesDelta);
        }
    }

    /**
     * @notice This function records full and partial withdrawals on behalf of one of the Ethereum validators for this BionicPod
     ...
     * @param beaconChainETHStrategyIndex is the index of the beaconChainETHStrategy for the pod owner for the callback to 
     *        the BionicPodManager to the StrategyManager in case it must be removed from the podOwner's list of strategies
    ...
    function verifyAndProcessWithdrawals(
        ...
        external
        onlyWhenNotPaused(PAUSED_BIONICPODS_VERIFY_WITHDRAWAL)
        onlyNotFrozen
    {
        require(
            (validatorFields.length == validatorFieldsProofs.length) && 
            (validatorFieldsProofs.length == withdrawalProofs.length) && 
            (withdrawalProofs.length == withdrawalFields.length), "BionicPod.verifyAndProcessWithdrawals: inputs must be same length"
        );
     ...
     */
    function decrementWithdrawableRestakedExecutionLayerGwei(uint256 amountWei) external onlybionicPodManager {
        uint64 amountGwei = uint64(amountWei / GWEI_TO_WEI);
        require(withdrawableRestakedExecutionLayerGwei >= amountGwei,
            "BionicPod.decrementWithdrawableRestakedExecutionLayerGwei: amount to decrement is greater than current withdrawableRestakedRxecutionLayerGwei balance");
     ...
     */
    function incrementWithdrawableRestakedExecutionLayerGwei(uint256 amountWei) external onlybionicPodManager {
        uint64 amountGwei = uint64(amountWei / GWEI_TO_WEI);
        withdrawableRestakedExecutionLayerGwei += amountGwei;
    }
        ...
        returns (uint256)
    {
        ...
        require(validatorInfo.status == VALIDATOR_STATUS.INACTIVE,
            "BionicPod.verifyCorrectWithdrawalCredentials: Validator must be inactive to prove withdrawal credentials");

        require(validatorFields[BeaconChainProofs.VALIDATOR_WITHDRAWAL_CREDENTIALS_INDEX] == bytes32(_podWithdrawalCredentials()),
            "BionicPod.verifyCorrectWithdrawalCredentials: Proof is not for this BionicPod");
        /**
        ...
        * actual validator balance by 0.25 ETH.  In Bionic, we calculate our own "restaked balance" which is a further pessimistic
        ...
        proofIsForValidTimestamp(Endian.fromLittleEndianUint64(withdrawalProofs.timestampRoot))
    {
         ...
         * and thus we cannot know that the validator is related to this BionicPod at all!
         */
        require(_validatorPubkeyHashToInfo[validatorPubkeyHash].status != VALIDATOR_STATUS.INACTIVE,
            "BionicPod._verifyAndProcessWithdrawal: Validator never proven to have withdrawal credentials pointed to this contract");
        require(!provenWithdrawal[validatorPubkeyHash][Endian.fromLittleEndianUint64(withdrawalProofs.slotRoot)],
            "BionicPod._verifyAndProcessWithdrawal: withdrawal has already been proven for this slot");

    
        // verify that the provided state root is verified against the oracle-provided latest block header
        BeaconChainProofs.verifyStateRootAgainstLatestBlockHeaderRoot(
            withdrawalProofs.beaconStateRoot,
            bionicPodManager.getBeaconChainStateRoot(oracleTimestamp),
    ...
    function _processFullWithdrawal(
        ...
        */
        if (status == VALIDATOR_STATUS.ACTIVE) {
            // if the withdrawal amount is greater than the MAX_VALIDATOR_BALANCE_GWEI (i.e. the max amount restaked on Bionic, per ETH validator)
            ...
            if (withdrawalAmountGwei > maxRestakedBalanceGwei) {
                ...
                // and the extra execution layer ETH in the contract is MAX_VALIDATOR_BALANCE_GWEI, which must be withdrawn through Bionic's normal withdrawal process
                withdrawableRestakedExecutionLayerGwei += maxRestakedBalanceGwei;
                withdrawalAmountWei = maxRestakedBalanceGwei * GWEI_TO_WEI;
                
            } else {

                // otherwise, just use the full withdrawal amount to continue to "back" the podOwner's remaining shares in Bionic (i.e. none is instantly withdrawable)
            ...
            revert("BionicPod.verifyBeaconChainFullWithdrawal: VALIDATOR_STATUS is invalid VALIDATOR_STATUS");
        }
     ...
     * @notice Called by BionicPodManager to withdrawBeaconChainETH that has been added to the BionicPod's balance due to a withdrawal from the beacon chain.
    ...
    */ 
    function activateRestaking() external onlyWhenNotPaused(PAUSED_BIONICPODS_VERIFY_CREDENTIALS) onlyBionicPodOwner hasNeverRestaked {
        hasRestaked = true;
    ...
    function withdrawBeforeRestaking() external onlyBionicPodOwner hasNeverRestaked {
        _processWithdrawalBeforeRestaking(podOwner);
    }
